...
function openAccountSwitcher() {
    document.getElementById('accountSwitcherOverlay').style.display = '';
    sendToCS({ action: 'getSavedAccounts' });
}
...
function renderAccountSwitcher(accounts) {
    ...
    if (!list) return;
    if (!_savedAccounts.length) {
        list.innerHTML = `<div class="acct-sw-empty">${t('account_switcher.no_accounts', 'No saved accounts yet. Log in to save an account.')}</div>`;
        return;
    }
    list.innerHTML = _savedAccounts.map(a => {
        const uid   = esc(a.userId || a.UserId || '');
        const name  = esc(a.displayName || a.DisplayName || '');
        ...
        return `<div class="acct-sw-item" onclick="doSwitchAccount('${uid}','${uname}')">
            ...
            <div class="acct-sw-actions">
                <button class="acct-sw-switch-btn" onclick="event.stopPropagation();doSwitchAccount('${uid}','${uname}')" data-i18n="account_switcher.switch">Switch</button>
                <button class="acct-sw-remove-btn" onclick="event.stopPropagation();removeAccount('${uid}')" title="${t('account_switcher.remove','Remove account')}"><span class="msi" style="font-size:16px;">close</span></button>
            </div>
        </div>`;
    }).join('');
...
function addNewAccount() {
    closeAccountSwitcher();
    sendToCS({ action: 'switchAccount', username: '' });
}
